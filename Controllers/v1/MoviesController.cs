...
namespace APILocacao.Controllers
{
    ...
    public class MoviesController : ControllerBase
    {
        ...
        [HttpPatch]
        public IActionResult Patch([FromBody] MovieDTO MovieDTO)
        {
            if (MovieDTO.Id > 0)
            {
                try
                {
                    var movie = _context.Movies.FirstOrDefault(movie => movie.Id == MovieDTO.Id);
                    if (movie != null)
                    {

                        movie.Name = MovieDTO.Name ?? movie.Name;
                        movie.Director = MovieDTO.Director ?? movie.Director;
                        movie.Synopsis = MovieDTO.Synopsis ?? movie.Synopsis;
                        movie.Amount = MovieDTO.Amount != 0 ? MovieDTO.Amount : movie.Amount;
